...
<div class="main">
    ...
    <div class="container">
        <div class="blog" id="posts">
            <h2 class="h2">Elimina los comentarios del Post</h2>
...
<script type="text/javascript">
                        ...
                        <div  id="com">
                            ...
                            <div class="tags">
                                <div class="wrapper btn-comentario">
                                    <p class="text-sm">
                                        <button onClick="responder(` + value[index].id + `)" class="blog-topic text-tiny"><i class="fa-solid fa-reply-all"></i> Responder</button>
                                        <button onClick="eliminarComentarioPadre(` + value[index].id + `)" class="blog-topic-delete-comentario text-tiny">Eliminar</button>
                                    </p>
                                </div>
                            </div>
    ...
    function enviarComentario(){
        ...
        $.ajax({
            ...
            url: "{{route('post.comentarios.admin')}}",
            type: 'post',
    ...
    function enviarRespuesta(){
        ...
        $.ajax({
            ...
            url: "{{route('post.respuesta.admin')}}",
            type: 'post',
            ...
            success: function(value){
                ...
                $.each(value, function(index) {
                    ...
                    respuesta += `<div class="respuesta-comentario">
                                    ...
                                    <p class="text-sm text-sm-comentario">` + value[index].texto + `</p>
                                    <p class="text-sm text-sm-comentario">
                                        <button onClick="eliminarComentarioHijo(` + value[index].id + `)"  class="blog-topic-delete-respuesta text-tiny">Eliminar</button>
                                    </p>
                                </div>`;
    ...
    function eliminarComentarioPadre(id){

        if (confirm("¿Desea eliminar este comentario?") == true) {

            $.ajax({
                type: 'POST',
                url: "{{route('comentario.eliminar')}}",
                data: {
                    id: id,
                    _token:$('input[name="_token"]').val()
                },
    ...
    function eliminarComentarioHijo(id){

        if(confirm("¿Desea eliminar este comentario?") == true){

            $.ajax({
                type: 'POST',
                url: "{{route('respuesta.eliminar')}}",
                data: {
                    id: id,
                    _token:$('input[name="_token"]').val()
                },
